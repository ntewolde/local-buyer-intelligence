...
async def create_intelligence_report(
    ...
    # Validate geography
    geography = db.query(Geography).filter(Geography.id == report_data.geography_id).first()
    if not geography:
    ...
    zip_demand_scores = engine.calculate_zip_demand_scores(zip_code_ids, service_category)
    ...
    channel_recommendations = _generate_channel_recommendations(
        buyer_profile, service_category
    )
...
def _generate_channel_recommendations(
    ...
    """Generate channel recommendations based on buyer profile"""
    recommendations = []
    ...
    if income_dist.get("high", 0) > income_dist.get("low", 0):
        recommendations.append({
            ...
            "estimated_reach": buyer_profile.get("target_households", 0) * 3,  # 3x reach
